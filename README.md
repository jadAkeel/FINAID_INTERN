...
Leakage-safe research pipeline for predicting the next monthly direction of 50 anonymous indicators (X1..X50). The active model is **Regime Adaptive Bidirectional Selector** with policy `forward_breadth_graduated_15_to_20` — selects **15-20** indicators per month (Up/Down) using `t-2` labels and `t-1` features only.

## Proven Results (Baseline)

From `research/regime_adaptive_selector/artifacts/predictions.parquet` (6349 rows 120-266):
...
```
configs/
  active_model.yaml               # active: forward_breadth_dynamic_cap_v3
  regime_adaptive_selector.yaml   # graduated 0.52->15 / 0.68->20, group weight 0.25
  config.yaml                     # data_path: data/monthly_indicators.xlsx (316 pos thru 2026-05-29)
src/forecast_select/
  regime_adaptive.py              # risk_adjusted = p_up*(1-0.25*stress)-0.15*p_down
  regime_adaptive_pipeline.py     # group 12m lag2 + graph 48m
research/
  regime_adaptive_selector/       # Baseline reference
  february_holdout_experiment/    # March-May holdout
  regime_adaptive_selection_group_v2/ # Latest research
reports/
  regime_adaptive_next_three_forecast.json # June-Aug forecast
```
...
# 3. Build active model (thru May 316)
python -m forecast_select.cli build-regime-adaptive

# 4. Show results
python -m forecast_select.cli show-regime-adaptive

# 5. Forecast June (H1) + July (H2) + August (H3) - each horizon trains thru t-2-(h-1)
...
`forecast-regime-next-three` writes `reports/regime_adaptive_next_three_forecast.json` — each horizon selects 15-20 indicators with `p_up`, `p_down`, `selection_score`, `group`, `rank`, `regime`.

Use `--cap` to override monthly cap (15-20):
...
## Current Run Output (2026-06 / 07 / 08)

Executed on `2026-05-29` Origin 316:
...
Full file: `reports/regime_adaptive_next_three_forecast.json` (15 selections per month 6/7/8 with `forecast_market_breadth 0.530562` and `regime_stress 0.487`).
...
All metrics cap-matched 15-20, `fit_through <= origin-2`, `locked 268-315` not read.
...
research/february_holdout_experiment/
