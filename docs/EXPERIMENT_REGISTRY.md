...
### 2.10 Calendar-Seasonal Prior (`seasonal_prior`)
- **Hypothesis**: An indicator's Up-frequency in the target's calendar month carries directional information beyond its trailing 48-label Up-rate. Motivation: every lagged feature in `features.py` is relative to the latest *known* change (t-2 → t-1), so `direction_lag_12` is the t-14 → t-13 move, two months off the target's season (t → t+1, counterpart t-12 → t-11), and no prior experiment conditioned on calendar month.
- **Status**: `rejected`. **Pre-registered** in `733119e` before any result existed.
- **Code Paths**: `research/seasonal_prior/seasonal_prior.py`
- **Config Paths**: N/A (all constants are in the script and were frozen at pre-registration)
- **Artifact Paths**: `research/seasonal_prior/metrics/monthly_records.csv`, `seasonal_strength.csv`
- **Report Paths**: `research/seasonal_prior/README.md`, `metrics/summary.json`, `existence_tests.json`, `window_results.csv`
- **Frozen Configuration**: Primary `H1_overlay` = production `selection_score` + 0.5 × an empirical-Bayes shrunk same-calendar-month Up-rate deviation. 0.5 is production's `indicator_prior_weight`; the seasonal variance is a pooled method-of-moments estimate at each origin, so nothing is tuned. Every candidate is compared at production's own per-origin call count and universe, keeping production's Down call on any row both select.
- **Observed Metrics**: Existence tests on Tuning-era labels (37 indicators, 2,000 joint permutations): pooled heterogeneity p = 0.314, **split-half reliability r = 0.006** (p = 0.467), breadth seasonality p = 0.887. `H1_overlay` vs production: Tuning −9 (p10 −12, p90 −2), Validation −4 (−6, −1), Confirmation −2 (−6, +2). Unshrunk version: −27 / +1 / −15. Control `prior_only` (production's prior alone): **−13 / +1 / −13**, with Tuning and Confirmation p10–p90 intervals entirely below zero. `window_ensemble`: −20 / **+11** (p10 +5) / −6.
- **Rejection / Decision Reason**: The gate fails on all four data criteria. Seasonality is absent, not merely weak: the method-of-moments seasonal SD of 3–5 points does not replicate across years. It comes from common monthly shocks that hit every indicator's cell for that month at once, which binomial sampling variance ignores. `window_ensemble`'s Validation gain is contradicted by both other windows and is a secondary arm that the pre-registration forbids promoting.
- **Secondary Findings**: The control arm is the first paired, matched-coverage measurement of production's model stack against its own base rate. The logistic, graph, and group overlay add +13 hits on Tuning and +13 on Confirmation, roughly 1.2–1.6 points, with intervals above zero, and nothing on Validation. The trailing-48 Up-rate implementation equals production's `indicator_prior` exactly (max difference 0.0).
- **Leakage & Holdout Warning**: Labels only through `t-2`. The workbook is read with `forecast_select.io.load_workbook(maximum_position=267)`, and the script asserts all 5,921 targets equal the production artifact's `y_true`. Existence tests use labels ≤ 177 only. Locked origins 268–315 were not read. Validation and Confirmation were viewed in earlier research, so this is not a fresh blind test.
- **Reproduction Command**: `python research/seasonal_prior/seasonal_prior.py`.
...
- **Observed Metrics**: 2,545 selected calls over 147 non-locked months, pooled accuracy 62.44% (Tuning 686/1071 = 64.05%, Validation 395/675 = 58.52%, Confirmation 508/799 = 63.58%). Six-month block bootstrap, 5,000 replicates: median 62.60%, p10–p90 **59.96%–65.11%**, p95 65.81%, P(≥ 62%) = 61.6%, **P(≥ 65%) = 11.3%**. Ranks 1–2 hit 66–69%, ranks 3–15 hit 54–65%, ranks 16–19 hit 59–62%. Expansion calls (16–20) are worse on Tuning (59.7%), equal on Validation (58.7%), and better on Confirmation (68.1%); a fixed 15-call policy on the delivered artifact scores 62.54% versus 62.44%, so dropping expansion is not a reliable gain.
- **Rejection / Decision Reason**: Not a promotion decision. The audit's central finding is that accuracy tracks the realized Up-prevalence of the selected set almost exactly by target year (2015: 46.7% / 46.7%; 2017: 74.8% / 74.8%; 2019: 76.2% / 76.7%; Jan–Apr 2022: 30.2% / 30.2%), because 2,534 of 2,545 calls are Up. Year-to-year accuracy swings (rolling-12 range 42.5%–76.2%) are market breadth, not model skill.
...
- **Status**: `active_research` (diagnostic; not a candidate). **Corrected 2026-09-23** — see Erratum.
...
- **Observed Metrics**: Production 64.05% / 58.52% / 63.58% (Tuning / Validation / Confirmation). Best zero-feature rule per window: N=48 63.56% on Tuning, N=60 61.17% on Validation, all-history 62.41% on Confirmation — a different window wins each time. Validation alone spans 56.17%–61.17% across window lengths. Fixed N=48: 572/900, 352/600, 435/705; this reproduces the independent local-logistic constant-`p_up` ablation exactly on Validation. Majority-direction (Down allowed) wins 2 of 9 comparisons, both on Tuning, and loses all six Validation and Confirmation comparisons. k=20 is below k=15 in all nine comparisons.
- **Rejection / Decision Reason**: Not a promotion decision. The unpaired spread across window lengths is dominated by month-level breadth and cannot separate rules. The paired, matched-coverage control arm in 2.10 (`prior_only`) is the precise measurement: production's model stack adds +13 hits on Tuning and +13 on Confirmation over its own prior, with p10–p90 intervals above zero, and −1 on Validation.
- **Erratum (2026-09-23)**: The first version (`0463c9b`) indexed workbook rows from 0, while the project numbers positions from 1 (`io.py`). Every window therefore ran one month late (for example, Tuning covered 121–180). Its Confirmation scored buffer origin 267, and it read level position 268, one row past the non-locked limit of 267. The evaluation remained causal (labels stopped two months before each target). **No locked outcome was read**: the first locked target (origin 268, position 268 → 269) needs position 269, which was never loaded. The error was caught by the seasonal-prior experiment's assertion that its targets equal the production artifact's `y_true`. The script now reads through `forecast_select.io.load_workbook(maximum_position=267)`.
